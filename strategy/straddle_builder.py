...
def _leg_metric_fields(result: Optional[dict]) -> dict:
    if not result:
        return {}
...
def _record_build_outcome(
    recorder: Optional[session_journal.EntryRecorder],
    ctx: session_journal.SessionCtx,
    result: str,
    reason: str = "",
    **extra,
) -> None:
...
async def build_straddle(
    ...
    session_journal.emit(
        ...
        strike=pair.strike,
        spot=entry_spot,
        call_bid=pair.call.bid, call_ask=pair.call.ask, call_mark=pair.call.mark,
        put_bid=pair.put.bid, put_ask=pair.put.ask, put_mark=pair.put.mark,
    )
    ...
    if rfq_result is not None:
        ...
        if put_result is not None and call_result is None:
            ...
            session_journal.emit(
                "entry_rollback", jctx,
                put_qty=put_qty_for_emer, call_qty=call_partial_qty,
                reason="call_leg_failed",
            )
            _record_build_outcome(
        ...
        if call_result is not None and put_result is None:
            ...
            session_journal.emit(
                "entry_rollback", jctx,
                call_qty=call_qty_for_emer, put_qty=put_partial_qty,
                reason="put_leg_failed",
            )
            _record_build_outcome(
        ...
        log.info("both_legs_filled", id=straddle_id, family=family.label(),
                 ...
                 unit=family.native_quote_unit_label())
        chase_secs = [
            float(put_metrics.get("duration_sec") or 0),
            float(call_metrics.get("duration_sec") or 0),
        ]
        session_journal.upsert_summary(jctx, entry_chase_sec=max(chase_secs))
