...
class OKXExchange:
    ...
    async def _taker_flatten_short(
        ...
        Mirror of ``_taker_flatten_long`` for the defensive case where a
        residual SHORT exists (e.g. a prior oversell). A maker buy-back can
        be stranded when the ask sits far above mark and the maker slippage
        cap never reaches it — the dying-0DTE wide-spread case. A TAKER buy
        that crosses the current ask closes the short IMMEDIATELY.
        ...
        try:
            ...
            try:
                ...
                await notifier.send(
                    ...
                    f"a long).\n"
                    f"Symbol: {instrument}\n"
                    f"Bought: {qty_btc:.4f} {config.BASE_COIN} @ ${ask:,.4f} (taker)"
                )
            except Exception:
