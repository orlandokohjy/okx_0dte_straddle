...
class Straddle:
    ...
    # 0.0 means "same as call strike" (legacy ATM straddle). Cosmetic only:
    # never used in P&L.
    ...
    # leg, a partial fill), we DEFER finalization: the straddle stays
    # ``open`` while the post-close re-flatten closes the residual, and
    # ``close_straddle`` is called only once flat. Each leg that actually
    # closes (unwind OR re-flatten) records its native fill price here so
    # the deferred finalize books REAL exits, never the entry-price
    # placeholder that produced phantom "closed" summaries.
